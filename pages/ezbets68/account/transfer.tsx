...
const transfer = () => {
  ...
  const columns = ['date', 'send', 'receive', 'amount', 'coin', 'action']
  const data = []
  ...
  React.useEffect(() => {
    ...
    for (let i = 0; i < 5; i++) {
      var itm = [
        { value: '2022-11-09-05 05:15:03', color: '' },
        { value: 'ETH', color: '' },
        { value: 'dsdsdse', color: '' },
        { value: (Math.random(100) * 100).toFixed(2), color: '' },

        { value: 'VND', color: '' },
        {
          value: (
            <select className={`${styles.withdraw_page_select_action_for_the_table}`}>
      ...
      <div className='mb-3 pb-5'>
          ...
          <Row className={`col-12 mx-0 ${styles.player_report_row}`}>
            <Col className='px-5 pb-4 col-12 text-center '>
              <div className={`${styles.player_report_text_container} `}>
                <span className={`${styles.player_report_text} `}> Transfer</span>
              </div>
            </Col>
          </Row>
        </div>
        ...
        <Row className={`mx-0 my-2 ${styles.player_report_row_background}`}>
          <div className={`${styles.player_report_table_mainHeading} text-center p-2`}>
            <label className={`${styles.font18} text-light ${styles.fw700}`}>Transfer List Infromation</label>
          </div>
